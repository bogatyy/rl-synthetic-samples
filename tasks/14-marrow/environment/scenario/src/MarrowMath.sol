...
library MarrowRangeMath {
    function liquidityFor(uint128 cashAmount, uint128 baseAmount, int24 lower, int24 upper)
        internal
        pure
        returns (uint256)
    {
        ...
        uint256 balanced = cashAmount < baseAmount ? cashAmount : baseAmount;
        // Narrow concentrated ranges represent substantially more active
        // liquidity than their token inventory.
        return balanced * 400 / width;
    }
}

library MarrowTickMath {
    ...
    function priceAtCompressedTick(int24 tick) internal pure returns (uint256) {
        uint128 ratio = ratioAtTick(tick * 100);
        return uint256(ratio) * uint256(ratio) * ONE >> 128;
    }

    function compressedTickAtPrice(uint256 priceWad) internal pure returns (int24 tick) {
        int24 low = -2_000;
        int24 high = 2_000;
        while (low < high) {
            int24 middle = low + (high - low + 1) / 2;
            if (priceAtCompressedTick(middle) <= priceWad) low = middle;
            else high = middle - 1;
        }
        tick = low;
    }
